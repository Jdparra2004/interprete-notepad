// =============================
// 0. Validar conexión con Backend
// =============================
fetch("http://127.0.0.1:5000/health")
    .then(r => r.json())
    .then(data => console.log("%cBackend conectado OK:", "color: green", data))
    .catch(err => console.error("%cError conectando al backend:", "color: red", err));


// =============================
// 1. Config
// =============================
const API_BASE = "http://127.0.0.1:5000";


// =============================
// 2. Referencias a elementos
// =============================
const el = {
    input: document.getElementById("input"),
    output: document.getElementById("output"),
    translateBtn: document.getElementById("translateBtn"),
    clearBtn: document.getElementById("clearBtn"),
    copyBtn: document.getElementById("copyBtn"),
    langLabel: document.getElementById("langLabel"),
    useGlossary: document.getElementById("useGlossary"),
    processingOverlay: document.getElementById("processingOverlay") // PASO 9
};


// =============================
// 3. Utilidad: Mostrar/Ocultar overlay "procesando"
// =============================
function setProcessing(state) {
    if (!el.processingOverlay) return;
    el.processingOverlay.style.display = state ? "flex" : "none";
}


// =============================
// 4. Función principal: traducir
// =============================
async function translate() {
    const text = el.input.value.trim();
    if (!text) return alert("Escribe algo para traducir.");

    // PASO 9 – mostrar overlay
    setProcessing(true);

    el.translateBtn.disabled = true;
    el.translateBtn.textContent = "Traduciendo...";

    try {
        const resp = await fetch(`${API_BASE}/translate`, {
            method: "POST",
            headers: { "Content-Type": "application/json; charset=utf-8" },
            body: JSON.stringify({
                text,
                use_glossary: el.useGlossary?.checked || false  // PASO 10
            })
        });

        const payload = await resp.json();

        if (!resp.ok) throw new Error(payload.error || "Error al traducir");

        // PASO 11 – asignar resultado
        el.output.value = payload.translated_text;
        el.langLabel.textContent = "Idioma detectado: " + (payload.detected_source || "—");

    } catch (err) {
        console.error(err);
        alert("Error: " + (err.message || err));
    } finally {
        // Restaurar estado UI
        el.translateBtn.disabled = false;
        el.translateBtn.textContent = "Traducir →";
        setProcessing(false); // PASO 9
    }
}


// =============================
// 5. Eventos UI
// =============================

// Botón traducir
el.translateBtn.addEventListener("click", translate);

// Botón limpiar
el.clearBtn.addEventListener("click", () => {
    el.input.value = "";
    el.output.value = "";
    el.langLabel.textContent = "Idioma detectado: —";
});

// Botón copiar
el.copyBtn.addEventListener("click", async () => {
    try {
        await navigator.clipboard.writeText(el.output.value || "");
        alert("Copiado al portapapeles");
    } catch {
        alert("No se pudo copiar");
    }
});

// Atajo Ctrl+Enter -> traducir
el.input.addEventListener("keydown", e => {
    if ((e.ctrlKey || e.metaKey) && e.key === "Enter") {
        translate();
    }
});
